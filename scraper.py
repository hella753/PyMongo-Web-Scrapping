    ...
    async def get_recipe_info(self) -> None:
        ...
        for i, response in enumerate(responses):
            ...
            category = self.soup_recipe.select_one(
                "body > div.container > div.pagination > div > " "div > a:nth-child(3)"
            )
            ...
            description = self.soup_recipe.find("div", {"class": "post__description"})
            ...
            portion = (
                self.soup_recipe.find("div", {"class": "lineDesc"})
            ...
            for ingredient in ingredient_elements:
                text = ingredient.text.replace("\xa0", "").replace("\n", "").strip()
                cleaned_text = " ".join(text.split())
                ingredients.append(cleaned_text)

            preparation = self.soup_recipe.find("div", {"class": "lineList"})
            preparation_elements = preparation.findAll(
                "div", {"class": "lineList__item"}
            )
            preparation_steps = {}
            for preparation_element in preparation_elements:
                preparation_step_count = preparation_element.find(
                    "div", {"class": "count"}
                ).text.strip()
                preparation_element_text = (
                    f'{preparation_element.find("p").text.strip()}'.replace(
                        "\n", " "
                    ).replace("\r", "")
                )
                preparation_steps[preparation_step_count] = preparation_element_text
